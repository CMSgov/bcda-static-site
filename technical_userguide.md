...
1) `POST` to https://api.bcda.cms.gov/auth/token using Basic Authentication
...
You can follow the steps for authenticating and receiving a token using Swagger, or follow the walk-through below if you are using the command line or terminal.
...
## Encryption
All data files are encrypted. You can learn more about the encryption strategy, and follow an example of how to decrypt using Python.
...
To obtain the exported explanation of benefit data, make a GET request to the output URLs in the job status response when the job reaches the Completed ```(200 OK)``` state. The data will be presented as an NDJSON file of ExplanationOfBenefit resources.
...
curl https://api.bcda.cms.gov/data/42/DBBD1CE1-AE24-435C-807D-ED45953077D3.ndjson \
   -H 'Authorization: Bearer {token}'
```
Take note of the {token}, be sure not to include the brackets {} for your request.

##### Response
The response will be the requested data as FHIR ExplanationOfBenefit resources in NDJSON format, encrypted. You can follow these instructions to decrypt.

An unencrypted example of one such resource is shown below. [add link out to pending About the Data page--does not exist yet]
...
To obtain the patient data, a ```GET``` request is made to the output URLs in the job status response when the job reaches the Completed state. The data will be presented as an NDJSON file of Patient resources.
...
##### Response
The response will be the requested data as FHIR Patient resources in NDJSON format, encrypted. An unencrypted example of one such resource is shown below. You can follow these instructions to decrypt. [link out to data dictionary/about the data on Patient resource]
...
Coverage data indicates the beneficiary’s enrollment record. It includes information on a beneficiary’s Part A, Part B, Part C, and Part D coverage, as well as markers for End-stage Renal Disease (ESRD) and Old Age and Survivors Insurance (OASI).
...
##### Response
The response will be the requested data as FHIR Coverage resources in NDJSON format. An example of one such resource is shown below. You can follow these instructions to decrypt. [link out to about the data/data dictionary on coverage resource]
