...
---
This page is intended for a user who has little to no experience with APIs, and provides a guided walkthrough for working with BCDA using our interactive documentation. More advanced API users may be better served by the [Advanced User Guide](/sandbox/technical-userguide/){:target="_self"}. If you’re not sure where to go, start here!
...
To get to BCDA's Swagger page for sandbox data, click on the following link, or copy and paste it into your web browser:

[https://api.bcda.cms.gov/api/v1/swagger](https://sandbox.bcda.cms.gov/api/v1/swagger){:target="_blank"}
...
**Note: The `/Group` endpoint differs from the `/Patient` endpoint in one key area. When filtering data using `_since` from the `/Group` endpoint, you will receive claims data since the date of your choice for existing beneficiaries AND you will also receive 7 years of historical data for all beneficiaries that are newly attributed to your ACO. See [Filtering your requests using `_since`](https://bcda.cms.gov/sandbox/user-guide/#filtering-your-requests-using-_since){:target="self"} for details and examples.**

#### 4. Making your first request for beneficiary data
To get any bulk beneficiary data, you must first be authorized with BCDA. Make sure you’ve followed the steps above for [Setting up your credentials in Swagger](https://bcda.cms.gov/sandbox/user-guide/#setting-up-your-credentials-in-swagger){:target="_self"} before moving forward.
...
In this example, we’ll show a request for all three resource types in the `/Patient` endpoint. If you want to learn how to make a request for data from one resource type, jump to [step 4b: Making a Request for One Resource Type](https://bcda.cms.gov/sandbox/user-guide/#b-making-a-request-for-one-resource-type){:target="_blank"}.
...
See [Making Your First Requests for Data](https://bcda.cms.gov/sandbox/user-guide/#making-your-first-requests-for-data){:target=”_self”} for step-by-step instructions on how to pull your historical data.
...
Follow the previous instructions for [getting your data from an unfiltered request](https://bcda.cms.gov/sandbox/user-guide/#5-getting-your-data){:target=”_self”} to retrieve your files after a filtered bulk data request.